...
def run_genome(input_genome):
    ...
    for _ in genome:
        ...
        # if player completed level, reached goal and end early
        if lib.Celeste_P8_get_player_y() < -4:
            reached_goal = True
            break

    return {
            # use fallback pos of 8, 96 (starting pos) if last pos invalid
            "final_x": x_positions[-1] if x_positions else 10.0,
            "final_y": y_positions[-1] if y_positions else 96.0,
